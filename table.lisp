...
(defmacro do-keys ((key table &optional (return-value nil)) &body body)
  (let ((discard (gensym))
        (value (gensym)))
    `(progn (fold-table (lambda (,discard ,key ,value) (declare (ignore ,value)) ,@body) nil ,table)
            ,return-value)))

(defmacro do-table ((key value table &optional (return-value nil)) &body body)
  (let ((discard (gensym))
        (key (gensym)))
    `(progn (fold-table (lambda (,discard ,key ,value) (declare (ignore ,key)) ,@body) nil ,table)
            ,return-value)))
  ...
  (dolist (rep1 '(alist-table plist-table hash-table wttree-table))
    (dolist (rep2 '(alist-table plist-table hash-table wttree-table))
      (let ((table1 (make-instance rep1))
        ...
        (dotimes (i 5)
          (table/insert! table2 (+ i 5) (format nil "~r" (+ i 5))))
        (let ((table3 (table/union-merge table1 table2 (lambda (k v1 v2) (cons v1 v2)))))
          (dotimes (i 5)
            (assert (string= (table/lookup table3 i "nil") (format nil "~r" i))))
          (dotimes (i 2)
  ...
  (dolist (rep1 '(alist-table plist-table hash-table wttree-table))
    (dolist (rep2 '(alist-table plist-table hash-table wttree-table))
      (let ((table1 (make-instance rep1))
        ...
        (dotimes (i 5)
          (table/insert! table2 (+ i 5) (format nil "~r" (+ i 5))))
        (table/union-merge! table1 table2 (lambda (k v1 v2) (cons v1 v2)))
        (dotimes (i 5)
          (assert (string= (table/lookup table1 i "nil") (format nil "~r" i))))
        (dotimes (i 2)
